...
def pts2px_360(pts, W, H):
    eps = 1e-5
    pts = torch.clamp(pts / (torch.norm(pts, dim=-1, keepdim=True) + eps), -1 + eps, 1 - eps)
    theta = torch.atan2(pts[..., 0], (pts[..., 2] + eps))
    phi = torch.asin(pts[..., 1])
    
    i = W * 0.5 * (theta/torch.pi + 1) - 0.5
    j = H * (phi/torch.pi + 0.5) - 0.5
...
def get_fwd_bwd_cam2cams_modified(cam2worlds, indices, init):
    if init:
        fwd_cam2cams = get_cam2cams(cam2worlds, indices, 1)
        bwd_cam2cams = get_cam2cams(cam2worlds, indices, -1)
    else:
        fwd_cam2cams = get_cam2cams(cam2worlds, indices-1, 1)
        bwd_cam2cams = get_cam2cams(cam2worlds, indices, -1)
    return fwd_cam2cams, bwd_cam2cams
...
def get_pred_flow_360(pts, ij, cam2cams, W, H):
    new_pts = torch.transpose(torch.bmm(cam2cams[:, :3, :3], torch.transpose(pts, 1, 2)), 1, 2)
    new_pts = new_pts + cam2cams[:, None, :3, 3]
    
    new_ij = pts2px_360(new_pts, W, H)
    
    flow = new_ij - ij.float()
    cyclic_flow = flow.clone()
    cyclic_flow[:,:,0] -= W
    cyclic_flow[:,:,1] -= H

    flow[abs(flow) > abs(cyclic_flow)] = cyclic_flow[abs(flow) > abs(cyclic_flow)] 
    return flow

def proj_360(pts, cam2cams, W, H):
    new_pts = torch.transpose(torch.bmm(cam2cams[:, :3, :3], torch.transpose(pts, 1, 2)), 1, 2)
    new_pts = new_pts + cam2cams[:, None, :3, 3]
...
def compute_depth_loss(dyn_depth, gt_depth):
    ...
    error = (dyn_depth_norm - gt_depth_norm) ** 2
    return dyn_depth_norm, gt_depth_norm, error
...
class TVLossPrev(nn.Module):
    def __init__(self,TVLoss_weight=1):
        super(TVLossPrev,self).__init__()
        self.TVLoss_weight = TVLoss_weight
    ...
    def forward(self,x):
        ...
        if h_x > 1:
            tv += torch.pow((x[:,:,1:,:]-x[:,:,:h_x-1,:]),2).mean()
            tv_outside = torch.pow((x[:,:,1:,:]-x[:,:,:h_x-1,:]),2)[:,:,:(h_x//4),:].mean()
            tv_outside += torch.pow((x[:,:,1:,:]-x[:,:,:h_x-1,:]),2)[:,:,(3*h_x//4):,:].mean()
            tv += 10 * tv_outside
        if w_x > 1:
            tv += torch.pow((x[:,:,:,1:]-x[:,:,:,:w_x-1]),2).mean()            
            tv_outside = torch.pow((x[:,:,:,1:]-x[:,:,:,:w_x-1]),2)[:,:,:,:(w_x//4)].mean()
            tv_outside += torch.pow((x[:,:,:,1:]-x[:,:,:,:w_x-1]),2)[:,:,:,(3*w_x//4):].mean()
            tv += 10 * tv_outside
...
def rodrigues_rot(axis, theta):
    ...
    K = np.array([[0, -axis[2], axis[1]],
                  [axis[2], 0, -axis[0]],
                  [-axis[1], axis[0], 0]])
    rot = np.eye(3) + (1-np.cos(theta))*(K@K) + np.sin(theta)*K
    return rot
...
def smooth_poses_spline(poses, st=0.5, sr=4, median_prefilter=True):
    ...
    # Circular spin for a parallax effect in the middle of trajectory
    spin_idx = int(0.7 * len(t))
    spin_len = int(2 * 0.8 * len(t))
    moving_dir = t[spin_idx+1] - t[spin_idx-1]
    moving_dir /= np.linalg.norm(moving_dir, axis=-1)

    # Calculate the rotation axis and angle
    initial_normal = np.array([0., 0., 1.])
    rot_axis = np.cross(initial_normal, moving_dir)
    rot_axis /= np.linalg.norm(rot_axis)
    rot_angle = np.arccos(np.dot(initial_normal, moving_dir) / (np.linalg.norm(initial_normal) * np.linalg.norm(moving_dir)))

    # Apply the rotation matrix to the circle points    
    theta, r = np.linspace(0.5*np.pi, 4.5*np.pi, spin_len), 0.15
    elp = np.ones_like(theta)
    elp[int(0.5 * len(elp)):] = 2
    x_circle, y_circle, z_circle = elp * r * np.cos(theta), r * np.sin(theta) - r, np.zeros_like(theta)    
    rot_mat = rodrigues_rot(rot_axis, rot_angle)
    rotated_t = rot_mat @ np.vstack((x_circle, y_circle, z_circle))
    rotated_t += t[spin_idx][:,None]
    
    poses[:, 0] = -poses[:, 0]
    
    smooth_posesnp0 = np.stack([x, y, z, t], -1)
    smooth_posesnp = np.zeros((len(x) + spin_len, 3, 4))
    smooth_posesnp[:spin_idx+1] = smooth_posesnp0[:spin_idx+1]

    spin_x = x[spin_idx][None,:].repeat(spin_len, axis=0)
    spin_y = y[spin_idx][None,:].repeat(spin_len, axis=0)
    spin_z = z[spin_idx][None,:].repeat(spin_len, axis=0)
    spin_posesnps = np.stack([spin_x, spin_y, spin_z, rotated_t.transpose()],-1)
    smooth_posesnp[spin_idx+1:spin_idx+1+spin_len] = spin_posesnps
    smooth_posesnp[-(len(x) - spin_idx - 1):] = smooth_posesnp0[-(len(x) - spin_idx - 1):]
