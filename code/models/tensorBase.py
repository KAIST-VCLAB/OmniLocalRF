...
def alpha2weights(alpha, fin_alpha_block=True):
    if fin_alpha_block:
        alpha[:, -1] = 1
    T = torch.cumprod(
        torch.cat(
...
class TensorBase(torch.nn.Module):
    ...
    def sample_ray_contracted(self, rays_o, rays_d, is_train=True, N_samples=-1):
        ...
        # t_vals = (
        #     torch.linspace(0.0, N_samples - 1, N_samples, device=rays_o.device)[None] / (N_samples-1)
        # )
    ...
    def updateAlphaMask(self, gridSize=(200,200,200)):
        ...
        # mask = torch.zeros_like(alpha, device=alpha.device, dtype=torch.bool)
        # start, end = int(gridSize[0]/ 4),  int(gridSize[0] * 3/ 4)
        # mask[start:end,start:end,start:end] = 1
        # mask = ~mask
        # # alpha[mask] = 1
        # x = torch.linspace(-2,2,gridSize[0], device=alpha.device)
        # y = torch.linspace(-2,2,gridSize[0], device=alpha.device)
        # z = torch.linspace(-2,2,gridSize[0], device=alpha.device)
