...
def reconstruction(args):
    ...
    print("omnilocal_rfs.dyn_block_iter ",omnilocal_rfs.dyn_block_iter )
    ...
    while training:
        ...
        # if len(omnilocal_rfs.tensorfs) > 1 and bidirectional_optimization:
            ...
            # # ### Dynamic RGB supervision on dst frame while forward refinement
            # dyn_noise = 0.1 * torch.rand_like(dst_rgbs, device=args.device) - 0.05
            # loss_for_mask_rgb = loss_dyn_weight * ((torch.abs((dst_dyn_map[:,:3] - (dst_rgbs + dyn_noise).clamp(0,1))) * loss_weights.view(-1)[...,None]) / loss_weights.mean())
            # loss_for_mask_rgb = loss_for_mask_rgb.mean()
            # loss_forward += loss_for_mask_rgb
            # writer.add_scalar("train/loss_for_mask_rgb", loss_for_mask_rgb, global_step=iteration) 

            # ### Binary cross entropy while forward refinemnt
            # dyn_beta_weight = args.loss_dyn_binary
            # loss_for_bin = dyn_beta_weight * (dst_dyn_map[:,3]**2 * (1-dst_dyn_map[:,3])**2).mean()
            # loss_forward += loss_for_bin
            # writer.add_scalar("train/loss_for_bin", loss_for_bin, global_step=iteration)       
            ...
            loss_back_rgb_src = 0.025 * ((torch.abs((back_prev_src_rgb_map - back_src_rgbs)) * back_src_loss_weights.view(-1)[...,None]) 
                                * (1-back_prev_src_dyn_map[:,3:].clone().detach()) ## Exclude masked rays of src frame
                                # * (1-dst_dyn_map[:,3:].clone().detach()) ## Exclude masked rays of dst frame
                            # / loss_weights.mean())[valid_refine_mask] ## Compute loss only over the valid rays
