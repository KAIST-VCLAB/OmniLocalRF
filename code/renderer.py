...
from utils.utils import (compute_depth_loss, get_fwd_bwd_cam2cams, get_pred_flow, get_pred_flow_360, rgb_ssim, visualize_depth, draw_poses)
...
def render(
    ...
    for i, idx in tqdm(enumerate(idxs)):
        ...
        rgb_map, static_rgb_map, depth_map, directions, ij, _, _ = local_tensorfs(
            rays_ids,
            view_ids,
            W,
            H,
        ...
        if test:
            ...
            if test_dataset.all_fwd_flow is not None:
                ...
                pred_fwd_flow = get_pred_flow_360(pts, ij, fwd_cam2cams, W, H).cpu().numpy()
                pred_bwd_flow = get_pred_flow_360(pts, ij, bwd_cam2cams, W, H).cpu().numpy()
    ...
    #         fps = 20
    #         duration = 1000 * 1/fps
    #         imageio.mimwrite(f, np.stack(rgb_maps_tb), duration=duration, quality=6, format="mp4", output_params=["-f", "mp4"])
    #     with open(f"{savePath}/posevideo.mp4", "wb") as f:
    #         imageio.mimwrite(f, np.stack(poses_vis), duration=duration, quality=6, format="mp4", output_params=["-f", "mp4"])
    #     with open(f"{savePath}/depthvideo.mp4", "wb") as f:
    #         imageio.mimwrite(f, np.stack(depth_maps_tb), duration=duration, quality=6, format="mp4", output_params=["-f", "mp4"])
