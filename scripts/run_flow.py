...
def compute_fwdbwd_mask(fwd_flow, bwd_flow, alpha_1=0.05, alpha_2=0.5):
    ...
    bwd2fwd_flow = warp_flow(bwd_flow, bwd_flow)
...
if __name__ == '__main__':
    ...
    os.makedirs(f"{args.data_dir}/flow_mask2", exist_ok=True)
    ...
    for filename in tqdm(input_files):
        ...
        if prev_frame_torch is not None:
            ...
            mask_fwd = np.zeros(frame[..., 0].shape, dtype=int)
            mask_bwd = np.zeros(frame[..., 0].shape, dtype=int)
        ...
        cv2.imwrite(f"{args.data_dir}/flow_mask/fwd_{fbase}.png", mask_fwd.astype(np.uint8).copy()*255)
        cv2.imwrite(f"{args.data_dir}/flow_mask/bwd_{fbase}.png", mask_bwd.astype(np.uint8).copy()*255)
